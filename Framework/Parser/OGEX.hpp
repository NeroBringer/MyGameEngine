...
namespace My {
	std::ostream& operator<<(std::ostream& out, DDLEnum type)
	{
	...
	public:
		...
		virtual std::unique_ptr<BaseSceneNode> Parse(const std::string& buf)
		{
			...
			ODDL::DDLEnum result = (ODDL::DDLEnum)openGexDataDescription.ProcessText(buf.c_str());
			...
			else
			{
				...
				cout << "error line:" << openGexDataDescription.GetErrorLine() << endl;
			}
#endif	
