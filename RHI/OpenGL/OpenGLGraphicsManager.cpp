...
bool OpenGLGraphicsManager::SetShaderParameters(float* worldMatrix, float* viewMatrix, float* projectionMatrix)
{
	...
	// Set the world matrix in the vertex shaer.
...
bool OpenGLGraphicsManager::InitializeBuffers()
{
	...
	// Bind the vertex buffer and load the vertex (position ad color) data into the vertex buffer.
	...
	// Specify the location and format of the position of portion of the vertex buffer.
	glBindBuffer(GL_ARRAY_BUFFER, m_vertexBufferId);
...
bool OpenGLGraphicsManager::InitializeShader(const char* vsFilename, const char* fsFilename)
{
	...
	const char* _v_cstr = vertexShaderBuffer.c_str();
	glShaderSource(m_vertexShader, 1, &_v_cstr, NULL);
